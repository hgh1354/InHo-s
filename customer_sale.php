...
//검색 시작
	...
	if(isset($searchColumn) && isset($searchText)) {
		$searchSql = ' where de_sp_cate = "고객" and ' . $searchColumn . ' like "%' . $searchText . '%"';
	} else {
		$searchSql = ' where de_sp_cate = "고객"';
	}
//검색종료
	...
	$query ="SELECT * FROM depo_spend ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
				  ...
				  <table class="table table-bordered table-hover table-striped">
					<thead class="cf" style='background-color: #BDBDBD'>
					  <tr>
						<th>선택</th>
						<th>판매명</th>
						<th class="numeric">고객명</th>
						<th class="numeric">담당자</th>
						<th class="numeric">매장명</th>
						<th class="numeric">판매 날짜</th>
					...
					<tbody>
					  <?while($row = $conn->DBF()){?>
						<tr>
						  ...
						  <td data-title="판매명"><a href="./customer_sale_detail.php?pNo=<?echo $row['de_sp_code'];?>"> <?echo $row['de_sp_name'];?></a></td>
						  <td class="numeric" data-title="고객명"><?php if($row['com_name'] != null){echo $row['com_name'];} else if($row['com_name'] == null){echo "/NaN";} ?></td>
						  <td class="numeric" data-title="담당자"><?php if($row['in_out_m'] != null){echo $row['in_out_m'];} else if($row['in_out_m'] == null){echo "/NaN";} ?></td>
							...
							<td class="numeric" data-title="판매 날짜"><?php if($row['de_sp_date'] != null){echo $row['de_sp_date'];} else if($row['de_sp_date'] == null){echo "/NaN";} ?></td>
