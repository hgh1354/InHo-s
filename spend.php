<?
	...
	switch($sCase)
	{
		//날짜
		case 1:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and de_sp_cate = "출금"';
		break;

		//거래처
		case 2:
		$searchSql = ' where com_name like "%' .$de_com. '%" and de_sp_cate = "출금"';
		break;

		//담당자
		case 3:
		$searchSql = ' where in_out_m like "%' .$de_mana. '%" and de_sp_cate = "출금"';
		break;

		//날짜 + 거래처
		case 4:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and com_name like"%' .$de_com. '%" and de_sp_cate = "출금"';
		break;

		// 날짜 + 담당자
		case 5:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and in_out_m like"%' .$de_mana. '%" and de_sp_cate = "출금"';
		break;

		// 거래처 + 담당자
		case 6:
		$searchSql = ' where com_name like "%' .$de_com. '%" and de_sp_cate = "출금" and in_out_m like "%' .$de_mana. '%" and de_sp_cate = "출금"';
		break;

		// 날짜 + 거래처 + 담당자
		case 7:
		$searchSql = ' where de_sp_date >= "' .$date_from. '" and de_sp_date <= "' .$date_to. '" and com_name like "%' .$de_com. '%" and in_out_m like "%' .$de_mana. '%" and de_sp_cate = "출금"';
		break;
	}

	//페이징
	$query = "SELECT count(*) FROM depo_spend ".$searchSql; // 추후 수정
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
...
<body>
  <section id="container">
    ...
    <section id="main-content" style="min-height:800px;">
        ...
        <h3><a href="deposit.php"><i class="fa fa-angle-right"></i>출금 조회</a></h3>
					...
					<div class="col-lg-12" style="" id="print">
						<section id="no-more-tables">
							<table class="table table-bordered table-hover table-striped">
								...
								?>
								<tr>
									<td data-title="선택"><input type="checkbox" id="chk_info[]" name="chk_info[]" value="<?echo $row['de_sp_code'];?>"></td>
									<td data-title="출금명"><a href="./spendDetail.php?dNo=<?echo $row['de_sp_code'];?>"><?echo $row['de_sp_name'];?></a></td>
									<td class="numeric" data-title="매입코드"><?php if($row['in_out_code'] != null){echo $row['in_out_code'];}else if($row['in_out_code'] == null){echo "/NaN";}?></td>
									...
									<td class="numeric" data-title="출금 날짜"><?php if($row['de_sp_date'] != null){echo $row['de_sp_date'];}else if($row['de_sp_date'] == null){echo "/NaN";}?></td>
