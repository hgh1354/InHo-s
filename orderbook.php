...
//검색 끝
	...
	$query = "SELECT * FROM wine_order where order_cate ='발주' ".$searchSql;
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
   ...
   $limit = $page->getVar("limit");   // 가져올 레코드의 시작점을 구하기 위해 값을 가져온다. 내부로직에 의해 계산된 값
	...
	$query ="SELECT * FROM wine_order where order_cate ='발주' ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list "; //변수에 쿼리 저장
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
  ...
  <section id="container">
    ...
    <section id="main-content" style="min-height:800px;">
	  <section class="wrapper">
        <h3><i class="fa fa-angle-right"></i><a href="<?=$_SERVER['PHP_SELF']?>">발주 조회</a></h3>
		<div class="row mt">
		...
		<div class="row" style="text-align:right">
          <div class="col-lg-12" style="">
		    <button type="button" class="btn btn-default" onclick="location.href='orderbook_form.php'">신규</button>
