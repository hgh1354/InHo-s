...
//검색 끝
   ...
   $page->setDisplay("class","page-item");
	...
	$query = "SELECT * FROM order_detail INNER JOIN wine_order ON order_detail.order_code = wine_order.order_code WHERE wine_order.order_cate = '발주'".$searchSql." ORDER BY order_detail.idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
