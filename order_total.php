...
//검색 끝
	...
	$query = "SELECT count(*) FROM order_detail INNER JOIN wine_order ON order_detail.order_code = wine_order.order_code WHERE wine_order.order_cate = '주문'".$searchSql;
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
	$cnt = $conn->DBF();
   ...
   $page->setDisplay("class","page-item");
	...
	$query = "SELECT * FROM order_detail INNER JOIN wine_order ON order_detail.order_code = wine_order.order_code WHERE wine_order.order_cate = '주문'".$searchSql." ORDER BY order_detail.idx DESC LIMIT $limit, $list"; //변수에 쿼리 저장
	$conn->DBQ($query);
	$conn->DBE(); //쿼리 실행
...
<!--main content start-->
	<section id="main-content" style="min-height:1000px;">
		<section class="wrapper">
			<h3><i class="fa fa-angle-right"></i>주문 현황</h3>
			<div class="row mt">
				<div class="col-lg-12">
							...
							<table class="table table-bordered table-hover table-striped">
								...
								<tbody>
									...
									<tr>
									  ...
									  <td class="numeric" data-title="총수량"><?echo $row['order_cnt'];?><? echo "EA"?></td>
